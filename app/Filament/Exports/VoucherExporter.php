...
class VoucherExporter extends Exporter
{
    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('min_items')->label('Min. Item'),
            ExportColumn::make('max_discount')->label('Maks. Diskon'),
            ExportColumn::make('is_shipping_voucher')->label('Voucher Ongkir?'),
            ExportColumn::make('exclude_resellers')->label('Kecuali Reseller?'),
            ExportColumn::make('is_stackable')->label('Bisa Ditumpuk?'),
            ExportColumn::make('free_gift_product_id')->label('ID Produk Hadiah'),
            ExportColumn::make('usage_limit')->label('Kuota'),
            ExportColumn::make('max_uses_per_user')->label('Maks. Pakai per Akun'),
