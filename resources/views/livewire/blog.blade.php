<main class="site-main bg-[#fcf9f5] min-h-screen pb-0">
    ...
    <section class="max-w-[1440px] mx-auto px-6 lg:px-12 py-8 md:py-12">
        <a href="{{ url('/blog/' . $featuredPost->slug) }}" wire:navigate class="block relative w-full aspect-[4/3] md:aspect-[21/9] overflow-hidden group">
    ...
    <section class="max-w-[1440px] mx-auto px-6 lg:px-12 py-8 border-b border-[#e5e2de] mb-12">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            ...
            <div class="flex gap-6 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide text-[10px] font-mono tracking-widest uppercase text-[#615e57]">
                <button wire:click="setCategory('all')" class="whitespace-nowrap {{ $category === 'all' ? 'text-[#1c1c1a] font-bold border-b border-[#1c1c1a] pb-1' : 'hover:text-[#1c1c1a] transition-colors' }}">All</button>
                @foreach($categories as $cat)
                    <button wire:click="setCategory('{{ $cat->slug }}')" class="whitespace-nowrap {{ $category === $cat->slug ? 'text-[#1c1c1a] font-bold border-b border-[#1c1c1a] pb-1' : 'hover:text-[#1c1c1a] transition-colors' }}">{{ $cat->name }}</button>
                @endforeach
            </div>
        </div>
    </section>
    ...
    <section class="max-w-[1440px] mx-auto px-6 lg:px-12 pb-24">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-x-20 md:gap-y-32">
            ...
            @php
                $postsToLoop = $featuredPost ? $posts->skip(1) : $posts;
            @endphp
