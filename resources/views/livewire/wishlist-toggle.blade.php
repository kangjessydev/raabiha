<div x-data="{ active: @js($isInWishlist) }" class="{{ $isDetail && !$isSimple ? 'h-full' : '' }}">
    @if($isSimple)
    <button wire:click.prevent.stop="toggleWishlist" x-on:click="active = !active"
            class="flex items-center justify-center w-full h-full transition-colors focus:outline-none"
            :class="active ? 'text-red-500' : 'text-[#615e57] hover:text-[#1c1c1a]'"
