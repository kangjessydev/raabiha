...
<main class="site-main bg-[#fcf9f5] min-h-screen pt-12 md:pt-24 pb-0">
    @php
        ...
        $aboutTimelineSubtitle = \App\Models\SiteSetting::where('key', 'about_timeline_subtitle')->value('value') ?: 'Evolusi dari atelier kecil ke pemimpin global.';
    ...
    <section class="max-w-7xl mx-auto px-6 lg:px-12 mb-24 md:mb-32">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div class="relative order-2 md:order-1">
                ...
                <img src="{{ $resolveImage($aboutAtelierImage, asset('assets/images/about-atelier.png')) }}" alt="Raabiha Atelier Process" class="w-full h-auto object-cover shadow-lg aspect-[4/3] md:aspect-auto">
            </div>
            
            <!-- Text (Right on Desktop, Bottom on Mobile) -->
            <div class="order-1 md:order-2 flex flex-col justify-center">
                <p class="text-[#064e3b] text-[10px] font-mono uppercase tracking-[0.2em] mb-4">{{ $aboutAtelierTag }}</p>
    ...
    <section class="max-w-7xl mx-auto px-6 lg:px-12 mb-24 md:mb-32">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-16 lg:gap-24">
            ...
            <div class="flex flex-col justify-center mt-4 md:mt-0">
                <p class="text-[#064e3b] text-[10px] font-mono uppercase tracking-[0.2em] mb-4 md:mb-6">{{ $aboutValuesTitle }}</p>
                <div class="space-y-8">
                ...
                <div class="space-y-16 md:space-y-24">
                    @foreach($aboutTimeline as $index => $item)
                        ...
                        <div class="relative flex flex-col md:flex-row items-start md:items-center justify-between group">
                            <!-- Dot -->
                            <div class="absolute {{ $isEven ? 'left-[3.5px] md:left-1/2 top-[3px]' : 'absolute left-[7px] md:left-1/2 top-2' }} md:top-1/2 {{ $isEven ? 'w-[15px] h-[15px]' : 'w-2 h-2' }} rounded-full bg-[#1c1c1a] flex items-center justify-center md:-translate-x-1/2 md:-translate-y-1/2 transition-all duration-300 group-hover:scale-110 ring-4 ring-[#f4f0eb]">
                                @if($isEven)
                            ...
                            @if(!$isEven)
                                ...
                                <div class="w-full md:w-[45%] pl-10 md:pl-0 flex items-center md:justify-start">
                                    <span class="text-[#064e3b] font-mono text-sm tracking-widest bg-[#064e3b]/10 px-3 py-1 rounded-full">{{ $item['year'] }}</span>
                                </div>
                            @else
                                <div class="w-full md:w-[45%] pl-10 md:pl-0 flex items-center md:justify-end order-2 md:order-1 mt-3 md:mt-0">
                                    <span class="text-white font-mono text-sm tracking-widest bg-[#064e3b] px-3 py-1 rounded-full shadow-sm">{{ $item['year'] }}</span>
                                </div>
