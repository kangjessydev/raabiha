...
<div class="md:hidden {{ $transparent ? 'absolute top-0 left-0 right-0 z-50 bg-transparent border-none' : 'sticky top-0 z-50 bg-[#fcf9f5] border-b border-[#e5e5e5]' }} px-4 py-3 flex items-center justify-between">
    <div class="flex items-center gap-1">
        ...
        <!-- Title -->
        <h1 class="{{ $transparent ? 'bg-white/80 backdrop-blur-sm px-4 py-1.5 rounded-full shadow-sm text-[10px] font-mono font-bold tracking-[0.2em] uppercase' : 'text-[13px] font-semibold tracking-[0.15em] uppercase text-black' }}">
            {{ $title }}
        </h1>
    </div>
    ...
    @else
        <!-- Placeholder not needed when title is on the left, but keep for layout balance if desired. We'll leave it empty. -->
    @endif
</div>
