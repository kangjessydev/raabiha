...
<div class="md:hidden {{ $transparent ? 'absolute top-0 left-0 right-0 z-50 bg-transparent border-none' : 'sticky top-0 z-50 bg-[#fcf9f5] border-b border-[#e5e5e5]' }} px-4 py-3 flex items-center justify-between">
    ...
    @if($cart)
        <a href="{{ url('/cart') }}" wire:navigate class="flex items-center justify-center w-10 h-10 -mr-2 rounded-full {{ $transparent ? 'bg-white/80 backdrop-blur-sm shadow-sm hover:bg-white active:bg-white/90' : 'hover:bg-black/5 active:bg-black/10' }} transition-colors text-black relative">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
            <div class="scale-75 absolute -top-1 -right-1 origin-top-right">
                <livewire:cart-badge />
            </div>
        </a>
    @else
