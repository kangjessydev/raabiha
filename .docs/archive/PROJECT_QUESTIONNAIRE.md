...
- **Fitur Wajib (MVP):** Apa saja fitur utama yang **harus ada** sebelum web ini dirilis ke publik? (Misal: Checkout, Hitung Ongkir otomatis, Payment Gateway, Dashboard Pelanggan). Jawab: Membangun logika dasar e-commerce (Katalog, Cart, Checkout, Order, Pembayaran, Ongkir) menggunakan struktur custom Laravel yang disederhanakan dari prinsip WooCommerce. Termasuk fitur cek ongkir gratis, payment gateway Xendit, dan dashboard customer.
...
- **Struktur Frontend:** Saat ini kita menggunakan arsitektur Laravel murni. Jawab: Untuk stack publik, kita gunakan Laravel Blade + Tailwind CSS (agar ringan, cepat, dan SEO-friendly). Untuk Admin Panel, kita gunakan Filament yang berjalan secara terpisah. Tidak diperlukan state management rumit seperti Vue/Pinia di frontend publik.
